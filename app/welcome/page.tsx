        ...
        <div className="space-y-4">
          {(!allUsers || allUsers.length === 0) ? (
            ...
            allUsers.map((targetUser) => {
              // MBTI（英数字）と絵文字を自動判別
              const rawAvatar = targetUser.avatar_type || '😊'
              ...
              return (
                <div key={targetUser.id} className="bg-white p-5 md:p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col gap-5 hover:shadow-md hover:border-violet-200 transition-all">
                  ...
                  <div className="flex items-start gap-4 w-full">
                    <div className={`flex items-center justify-center rounded-full border border-violet-100 shrink-0 bg-violet-50 w-16 h-16 mt-1 ${isTextAvatar ? 'text-lg font-black text-violet-600 tracking-widest' : 'text-4xl'}`}>
                      {avatar}
                    </div>
